...
namespace CrashBonus.Main {
    ...
    public partial class GamePlayer : MyPlayer {
        ...
        public long TotalWin { get; private set; }//玩家總贏分(暫代)
        public long TotalBet { get; private set; }//玩家總下注(暫代)
        public float PlayerRTP {//玩家目前RTP(暫代)
            get {
                if (TotalBet == 0) return 0.95f;
                WriteLog.Log($"TotalWin: {TotalWin} TotalBet: {TotalBet} RTP: {(float)TotalWin / (float)TotalBet}");
                return (float)TotalWin / (float)TotalBet;
            }
        }
        public void AddPt(long _value) {
            Pt += _value;
        }
        public void AddTotalWin(long _value) {
            TotalWin += _value;
        }
        public void AddTotalBet(long _value) {
            TotalBet += _value;
        }

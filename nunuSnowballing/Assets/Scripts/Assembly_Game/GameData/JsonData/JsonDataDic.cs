...
namespace Scoz.Func {
    ...
    public partial class GameDictionary : MonoBehaviour {
        ...
        public static void LoadJsonDataToDic(Action _action) {
            ...
            JsonBase.SetData_Remote<JsonMap>(SetDic);
            JsonBase.SetData_Remote<JsonMonster>(SetDic);
            JsonBase.SetData_Remote<JsonEquip>(SetDic);
        ...
        public static Dictionary<int, T> GetIntKeyJsonDic<T>() where T : JsonBase {
            ...
            if (dataName == null) {
                WriteLog.LogError("T類反射後為null, 應該是LoadJsonDataToDic中沒有SetData過該Json類資料");
                return null;
            }
        ...
        /// <summary>
        /// 傳入IDs取得T類型的List<JsonData>
        /// </summary>
        public static List<T> GetJsonDatas<T>(List<int> _ids) where T : JsonBase {
            if (IntKeyJsonDic == null)
                return null;
            if (_ids == null || _ids.Count == 0) {
                WriteLog.LogError("GetJsonDatas傳入資料為空");
                return null;
            }
            string dataName = GetDataName<T>();
            if (dataName == null) {
            ...
            if (IntKeyJsonDic.ContainsKey(dataName) && IntKeyJsonDic[dataName] != null) {
                List<T> tList = new List<T>();
                foreach (var id in _ids) {
                    if (!IntKeyJsonDic[dataName].ContainsKey(id)) {
                        WriteLog.LogErrorFormat("{0} 表不存在ID為{1}的資料", dataName, id);
                        return null;
                    }
                    tList.Add(IntKeyJsonDic[dataName][id] as T);
                }
                return tList;
            }
            WriteLog.LogErrorFormat("{0} 表不存Dic資料", dataName);
            return null;
        }
